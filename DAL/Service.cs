    ...
    public class Service
    {
        ...
        public void WriteCategoryFile(List<Category> categories)
        {
            using (TextWriter tw = new StreamWriter(categoryFile))
            {
                ...
                {
                    tw.WriteLine(string.Format("{0}: Id= {0} - Category Name= {1}", categories[i].CategoryId,
            ...
            using (TextWriter tw = new StreamWriter(productFile))
            {
                foreach (var product in products)
                {
                    ...
                    tw.WriteLine(string.Format(
                        "Id: {0} - Product Name: {1} - Price: {2} - Category Id: {3} - Category Name: {4}", product.Id,
                        product.ProductName, product.Price, product.CategoryId, categoryName));
                }
            }
        }
